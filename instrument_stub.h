#pragma once
// instrument_stub.h — Bochs 2.0's own instrumentation-hook stub
// (instrument/stubs/instrument.h), used verbatim.
//
// NOTE (Bochs 2.0 port): the previous version of this file was a
// hand-written, 2.7-oriented macro list with several arities that
// didn't match this version's actual call sites (BX_INSTR_OPCODE,
// BX_INSTR_REPEAT_ITERATION, BX_INSTR_MEM_DATA, BX_INSTR_RESET, and
// more all take different argument counts here), and it was missing
// several macros 2.0 needs entirely (BX_INSTR_SHUTDOWN,
// BX_INSTR_NEW_INSTRUCTION, the PREFIX_* family, etc). It also had a
// BOCHS_GLUE-gated special case for BX_INSTR_INTERRUPT intended to
// let bochs_glue.cpp provide a real bx_instr_interrupt() hook for
// guest int 0x80 - grepping the current bochs_glue.cpp/kernel.cpp
// shows nothing actually calls or relies on that hook (guest exit /
// port-IO interception happens via bx_devices_c::outp on ports
// 0xE8/0xE9 instead - see bochs_infra.cpp), so that complexity was
// vestigial. Using Bochs's own real stub verbatim guarantees every
// arity matches this version's actual headers/call sites exactly,
// gated the same way Bochs itself gates it (#if BX_INSTRUMENTATION,
// which config.h already sets to 0 for this build - the #else/no-op
// branch is what's actually compiled).
