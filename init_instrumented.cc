...
// Added to localise a hang inside BX_CPU_C::initialize(). The kernel glue
// provides live_breadcrumb(slot, ch): it paints one glyph at row 0 col
// `slot` AND (when a `test` run is active) emits "[bc NN='c']" to the
// host debug console on port 0xE9. By calling it between each sub-step of
// initialize(), the LAST breadcrumb printed before the freeze names the
// exact step that hung. Slots 60..69 are used here so they don't collide
// with the glue's existing 30..42 range.
...
{
  live_breadcrumb(60, 'I');           // entered initialize()
#if BX_CPU_LEVEL >= 4
  BX_CPU_THIS_PTR cpuid = cpuid_factory(this);
  live_breadcrumb(61, 'c');           // cpuid_factory() returned
  ...
  live_breadcrumb(62, 'e');           // get_cpu_extensions() returned
  ...
  live_breadcrumb(63, 'f');           // init_FetchDecodeTables() returned

#if BX_CPU_LEVEL >= 6
  xsave_xrestor_init();
  live_breadcrumb(64, 'x');           // xsave_xrestor_init() returned
#endif
  ...
  live_breadcrumb(65, 'm');           // got MSR path, about to load_MSRs()
  load_MSRs(msrs_filename);
  live_breadcrumb(66, 'M');           // load_MSRs() returned
#endif
  ...
  init_SMRAM();
  live_breadcrumb(67, 's');           // init_SMRAM() returned
  ...
  init_statistics();
  live_breadcrumb(68, 'S');           // init_statistics() returned — initialize() done
}
